...
1. **`017` migration numbering collision.** Both `017_city_boundaries.sql` and `017_docs_rejected.sql` exist. `migrate.ts` tracks applied migrations by filename (confirm this before touching), so renaming is safe as long as the file has not yet run against a target DB in a way that recorded the old filename. Renumber `017_docs_rejected.sql` → `055_docs_rejected.sql` (rename the smaller/newer-content file, not the one more likely referenced elsewhere) and update the seed/comment cross-references if any exist.

2. **~25 leftover one-line stub files** in `src/` (controllers/services scaffolded but never filled in — real logic landed in bigger consolidated files instead, e.g. `payments.repository.ts` is a stub, the real logic is in `payments.service.ts` + submodules). Delete them. Confirm each is genuinely unreferenced (import-grep) before deleting, since `payments.repository.ts`'s stub-ness is intentional and referenced by module wiring even though empty — don't delete files that are still imported, just the ones with zero references.

3. **Duplicate rating columns.** `drivers` and `users` each carry both `total_ratings` (from `009_m7_safety.sql`) and `rating_count` (added later in `018`/`019` schema-align migrations). This is a decision (which one the app actually writes/reads), not an indexing fix — grep both column names across `src/modules/safety` and `src/modules/drivers`/`users` to see which one is live, then drop the dead one in a follow-up migration. Flagging, not resolving here.
