...
- Corrupt the state machine (e.g. calling `markArrived` on a `completed` ride, since no status precondition exists)
...
1. `markArrived` returns `{ startOtp }` directly in the driver's HTTP response (L362), so the driver doesn't need to ask the passenger for the code at all.
...
- Strip `start_otp_hash` and `end_otp_hash` from all API responses, since these columns should never leave the database
...
```
The `ride_otp_events` table already exists, so use it.
...
In the socket handler, `join:ride` checks `ride.user_id === callerSub || ride.driver_id === callerSub` without checking the role, so driver #42 can join the ride room for user #42's completely unrelated ride.
...
- Rental drops (`dropLat` null) result in `undefined`, so the backend `verifyEndOTP` never receives a valid actual distance and can't compute overage
...
If the driver's session isn't `'online'` at accept time (e.g. grace-period race marked it offline), `sessionId` is undefined and the function doesn't flip session to `'on_trip'`. The driver then remains eligible for new broadcasts while already on a trip. Additionally, `rides.session_id` and `rides.vehicle_id` are never written on accept, so trip history can't link a ride to the vehicle that served it.
...
No check for an existing active ride in `createBooking`. No unique partial index prevents it at the DB level. Each spurious booking fires 3 broadcast rounds × up to 5 drivers × ack-check retries, so a single double-tap generates ~30 BullMQ jobs.
...
Every `driver:location` socket event spreads the entire breadcrumb array into a new array. A 2-hour rental at one ping every 2 seconds accumulates 3,600 points, and each event triggers a full array copy plus re-render of the map component.
...
Rate card, stop charges, surge check, and rental package are separate sequential queries, and none are cached. Rate cards and stop charges change rarely (insert-only versioning by design), so they are ideal cache candidates.
...
`rides.validator.ts` is effectively empty. Every money-critical route does `req.body as BookingRequest` raw casting. The auth module and drivers module both have validators, and the rides module is the highest-value path with none.
...
- `findNearbyDrivers` becomes `GEORADIUS ocar:drivers:{cityId} lng lat 5 km`, microseconds instead of milliseconds
