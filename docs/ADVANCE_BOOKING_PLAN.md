...
proactively claim a scheduled trip hours ahead for a small guarantee bonus — this is the exception,
...
confirmed ride now carries a rider-payable penalty from the platform; surge is capped at 1.5×
...
The existing index (`007_m5_booking.sql:192-194`) filters `status = 'requested'` — that was written
...
(`002_enums.sql:75-77`) — none of these fit "cancelled while still scheduled, driver not even
...
2. Insert the ride with `status = 'scheduled'`, `scheduled_for = data.scheduledFor` — **skip** the
   `logStatusHistory('requested')` call and **skip** enqueueing `BroadcastJobData` entirely. This
...
worker-registration file name before wiring; it wasn't read in this pass).
...
- T-24h and T-1h: emit `ride:reminder` via socket if the user is connected (best-effort; most users
  ...
  flag it as such in code, don't pretend it's a real reminder channel yet).
...
3 scheduled rides simultaneously; not 4 live rides.
...
safe — whichever fires first wins, the second gets zero rows back and no-ops. This is the same
...
- Rate cards are versioned by design (`CLAUDE.md` invariant) specifically so prices can change —
  locking a quote for up to 7 days would mean carrying "V1 pricing debt" against future rate cards,
...
- Surfaces stuck bookings (e.g. `pending_driver` past its `scheduled_for` time — sweep failed) so
