...
**Fix:** collapse the orbs into one static blurred background image (or CSS `@keyframes` on `transform` only with `will-change: transform`), and drop the particles to CSS animations — framer-motion runs these through JS springs per frame.
...
```
Onboarding is visited once per driver lifetime — it is the textbook split point.
...
### 3. MEDIUM — Zustand stores consumed without selectors → broad re-renders, including at the App root
