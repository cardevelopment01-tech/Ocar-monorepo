...
**Native fan-out.** One job fans to N driver queues using BullMQ's `addBulk`. SQS requires SQS + SNS + Lambda to achieve the same result: three services, three billing dimensions, and higher operational complexity.
...
**Per-job TTL.** Broadcast offers expire in 15–20 seconds, after which the driver window closes and the job is irrelevant. BullMQ supports `removeOnComplete` and job expiry natively. SQS visibility timeout is a poor substitute.
...
**Redis already in stack.** Redis is required for OTP storage, GPS buffering, and session caching. BullMQ reuses the same Redis instance, with no additional infrastructure dependency.
...
- Requires a persistent Redis instance (`appendonly yes`). Managed options: Upstash (~$0/month for low volume, pay-per-request) and ElastiCache (~$20/month for cache.t3.micro).
...
If scale demands it, switch to Redis Cluster mode. BullMQ supports cluster natively via `ioredis` Cluster client, so no application-level code changes are required.
