...
- **Round-trip overage billing needs the driver app to report the real round-trip distance, which it doesn't yet.** `verifyEndOTP` (`api/src/modules/rides/rides.service.ts`) now reconciles round_trip fare against `actualDistanceKm`/`actualDurationMin` at trip completion, expecting `actualDistanceKm` to be the TOTAL to-and-fro distance driven. But `apps/driver/src/pages/ActiveRide/TripInProgress.tsx`'s `handleCompleteTrip` currently computes this as a one-way straight-line estimate to the destination (`haversine(pickup, dest) × 1.3`), not the actual round-trip total. Until the driver app is updated to report real round-trip distance (either by accumulating GPS distance client-side for round_trip rides, or by the API aggregating the `gps_tracks` table server-side at completion — that table is currently write-only, never read back for billing), overage-km billing will rarely trigger correctly in production even though the backend logic is correct. This is the actual functional gap to close next, not a cosmetic one. Delete this note once both the per-category rate-card tuning and the driver-app real-distance reporting are done.
