...
- **cAdvisor is confirmed live on the fleet** (verified directly via `docker ps` on a running instance during the 2026-08-30 blue/green rollout — `ocar_cadvisor` up and healthy, scraped fine by Alloy) — the `terraform apply` half of this rollout is done, most likely as an incidental side effect of that rollout's multiple instance refreshes, not a deliberate action. **Unconfirmed: whether `ocar-overview.json`'s "Per-Container Resource Usage" row has actually been re-imported into the live Grafana Cloud dashboard** — that's a manual UI-only step (see `infra/grafana/dashboards/README.md`) nothing in this repo can verify from the outside. If you can see that row with real data in Grafana, delete this note; if not, it's a 2-minute manual import, not a code change. Once real/load-tested traffic exists, use that row's data to size `api`'s `mem_limit`/`cpus` (currently 2g/512m/1.5, a starting safety-net guess) from observed p95/p99 usage.
