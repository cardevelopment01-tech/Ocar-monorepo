...
- **Before the client DB load test, enable observability in the Neon dashboard (can't be done from code):**
  ...
  After the test, run `api/scripts/index-usage-audit.sql` to find unused indexes, dead-tuple pressure, and the slowest query shapes — that data decides whether to build keyset pagination and `ride_status_history` partitioning (both deliberately deferred until proven necessary — see `docs/superpowers/specs/2026-07-26-db-loadtest-readiness-design.md`). If `ride_messages` (rider-driver in-app chat, once built) exists by then, include it in the same test — it's the same one-row-per-event/high-write/read-by-`ride_id` shape as `ride_status_history`, so it should get the same partitioning decision from the same data, not bespoke scaling work.
...
- **No durable log storage across deploys or servers — the "Grafana Alloy → Loki" plan referenced in `docs/superpowers/plans/2026-08-05-pino-structured-logging.md` was never validated or implemented; it doesn't exist anywhere else in the repo.** Pino already writes structured JSON to stdout with `requestId`/`correlationId` threaded through HTTP, Socket.io, and BullMQ (done). What's still missing is shipping that stdout somewhere durable — right now Docker's `json-file` driver (now rotated via `docker-compose.prod.yml`'s `logging:` block, `max-size: 10m` / `max-file: 5`, added 2026-08-05) is the only place logs live, and it's wiped on every container recreate/deploy. Researched 2026-08-05: self-hosting Loki+Alloy+S3+Grafana is over-engineering for this team's size and was rejected — recommendation is **Grafana Cloud Logs (hosted Loki, free tier ≤50GB/14-day retention) shipped via Fluent Bit** (1-5MB RAM, disk-buffered via `storage.type filesystem` so a backend hiccup queues logs locally instead of dropping them or blocking request handlers), with **AWS CloudWatch Logs via Docker's native `awslogs` driver** (`mode=non-blocking` to avoid stalling the app) as the zero-agent AWS-native fallback if already on EC2. Axiom's free tier (0.5TB/mo, 30-day retention) is also a legitimate $0 option if LogQL/Grafana portability isn't a priority. Do NOT self-host Loki/Alloy — no dedicated ops person to babysit it at this log volume. When implementing: keep `requestId`/`correlationId` as a queried JSON field, not a Loki label (label cardinality explosion) — only `service`/`host` should be labels. This matters more once multiple servers are running the same API image, since without shipping, each server's logs are siloed and require SSH-per-box to grep. Delete this note once a backend is picked and Fluent Bit (or the CloudWatch driver) is wired in.
