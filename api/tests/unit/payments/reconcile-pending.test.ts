...
// module file) — same finding documented in confirm-ride-payment.test.ts /
// verify-ride-payment.test.ts / webhook-captured.test.ts. Instead of trusting a
// spy, we assert on the real side effect: the guarded
...
// removing the confirmRidePayment call from reconcilePendingRidePayments made
// test 1 fail (no confirming UPDATE issued) and restoring it made it pass again.
  ...
  it('captured on recheck → confirms', async () => {
    vi.mocked(pool.query).mockImplementation(async (text: unknown) => {
      ...
      if (sql.includes("WHERE status = 'pending'") && sql.includes('razorpay_order_id IS NOT NULL')) {
        return { rows: [{ ride_id: 101, razorpay_order_id: 'order_1' }], rowCount: 1 } as never
      }
    ...
    expect(confirmCall![1] as unknown[]).toEqual([BigInt(101), 'pay_9'])
  })
