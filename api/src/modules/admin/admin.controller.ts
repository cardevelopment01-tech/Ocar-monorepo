...
export async function updateDriverStatus(
  ...
  try {
    ...
    const payload: { status: DriverStatus; reason?: string } = {
...
export async function updateDriverProfile(
  ...
  try {
    ...
    if (body["experience_years"] !== undefined) {
      ...
      if (isNaN(years))
        throw Object.assign(new Error("experience_years must be a number"), {
          httpStatus: 400,
        });
      payload.experience_years = years;
    }
    if (body["languages_known"] !== undefined)
      payload.languages_known = body["languages_known"] as string[];

    await service.updateDriverProfile(
      driverId,
      adminId,
      payload,
      req.ip ?? null,
...
export async function updateDriverVehicle(
  ...
  try {
    ...
    const driver = await service.getDriver(driverId);
    if (!driver.vehicle)
      throw Object.assign(new Error("Driver has no vehicle registered"), {
        httpStatus: 404,
      });

    const body = req.body as Record<string, unknown>;
    const payload: UpdateDriverVehiclePayload = {
      reason: String(body["reason"] ?? ""),
    };
    ...
    for (const field of VEHICLE_NUMBER_FIELDS) {
      if (body[field] !== undefined) {
        ...
        if (isNaN(n))
          throw Object.assign(new Error(`${field} must be a number`), {
            httpStatus: 400,
          });
        payload[field] = n;
      }
    }
    if (body["ac_availability"] !== undefined)
      payload.ac_availability = Boolean(body["ac_availability"]);
    if ("model_id" in body)
      payload.model_id = body["model_id"] ? String(body["model_id"]) : null;

    await service.updateDriverVehicle(
      driverId,
      BigInt(driver.vehicle.id),
      adminId,
      payload,
      req.ip ?? null,
    );
...
export async function getDriverRides(
  ...
  try {
    const result = await service.listDriverRides(
...
export async function getDriverPayments(
  ...
  try {
    const result = await service.listDriverPayments(
      BigInt(req.params["id"]!),
      pageQuery(req),
    );
    res.json(result);
...
export async function getDriverAuditLog(
  ...
  try {
    const result = await adminAuditService.listAuditLogForTarget(
      "drivers",
      BigInt(req.params["id"]!),
      pageQuery(req),
    );
...
export async function patchAdminStatus(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
...
export async function patchCategory(
  ...
  try {
    ...
    if (req.body.display_name !== undefined)
      catData.display_name = String(req.body.display_name);
    if (req.body.max_passengers != null)
      catData.max_passengers = Number(req.body.max_passengers);
    if (req.body.is_active !== undefined)
      catData.is_active = Boolean(req.body.is_active);
...
export async function patchBrand(
  ...
  try {
    ...
    if (req.body.is_active !== undefined)
      brandData.is_active = Boolean(req.body.is_active);
...
export async function getModels(
  ...
  try {
    res.json(
      await service.listModels(req.query["brand_id"] as string | undefined),
    );
  } catch (err) {
    next(err);
  }
...
export async function postModel(
  ...
  try {
    const model = await service.createModel({
      ...
      name: String(req.body.name ?? ""),
      typical_category_id: req.body.typical_category_id
        ? String(req.body.typical_category_id)
        : null,
      is_active: req.body.is_active !== false,
    });
...
export async function patchModel(
  ...
  try {
    ...
    if ("typical_category_id" in req.body)
      data.typical_category_id = req.body.typical_category_id
        ? String(req.body.typical_category_id)
        : null;
    if (req.body.is_active !== undefined)
...
export async function getFleet(
  ...
  try {
    res.json(
      await service.listFleet(req.query["status"] as string | undefined),
    );
  } catch (err) {
    next(err);
  }
...
export async function unblacklistVehicle(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
...
export async function getPendingVehicleDocs(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    res.json(await service.listPendingVehicleDocs());
  } catch (err) {
    next(err);
  }
}
...
export async function getAdminRateCards(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    res.json(await service.listAdminRateCards());
  } catch (err) {
    next(err);
  }
}

export async function getAdminRateCardHistory(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    res.json(await service.listAdminRateCardHistory());
  } catch (err) {
    next(err);
  }
}

export async function postAdminRateCard(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    ...
    if (req.body.return_rate_per_km !== undefined)
      data.returnRatePerKm =
        req.body.return_rate_per_km != null
          ? Number(req.body.return_rate_per_km)
          : null;
    if (req.body.hour_rate !== undefined)
      data.hourRate =
        req.body.hour_rate != null ? Number(req.body.hour_rate) : null;
    if (req.body.km_per_day !== undefined)
      data.kmPerDay =
        req.body.km_per_day != null ? Number(req.body.km_per_day) : null;
    if (req.body.driver_allowance_per_day !== undefined)
      data.driverAllowancePerDay =
        req.body.driver_allowance_per_day != null
          ? Number(req.body.driver_allowance_per_day)
          : null;
    if (req.body.city_id !== undefined)
      data.cityId = req.body.city_id != null ? Number(req.body.city_id) : null;
...
export async function getAdminSurgeEvents(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    res.json(await service.listAdminSurgeEvents());
  } catch (err) {
    next(err);
  }
}

export async function postAdminSurgeEvent(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    const surge = await service.createAdminSurgeEvent({
      ...
      categoryId:
        req.body.category_id != null ? Number(req.body.category_id) : null,
...
export async function cancelAdminSurgeEvent(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    res.json(
      await service.cancelAdminSurgeEvent(
        BigInt(req.params["id"]!),
        req.admin!.id,
      ),
    );
...
export async function getAdminCities(
  ...
  try {
    res.json(await service.listAdminCities());
  } catch (err) {
    next(err);
  }
}

export async function postAdminCity(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
...
export async function patchAdminCity(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    ...
    if (req.body.default_speed_limit_kmph !== undefined)
      data.default_speed_limit_kmph = Number(req.body.default_speed_limit_kmph);
    if (req.body.status !== undefined) data.status = String(req.body.status);
    if (req.body.is_rental_enabled !== undefined)
      data.is_rental_enabled = Boolean(req.body.is_rental_enabled);
    if (req.body.is_return_cab_enabled !== undefined)
      data.is_return_cab_enabled = Boolean(req.body.is_return_cab_enabled);
    if (req.body.billing_mode !== undefined)
      data.billing_mode = String(req.body.billing_mode) as
        | "commission"
        | "package";
...
export async function getPackageTiers(
  ...
  try {
    res.json(await service.listPackageTiers());
  } catch (err) {
    next(err);
  }
}

export async function postPackageTier(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
...
export async function patchPackageTier(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    ...
    if (req.body.thresholdValue !== undefined)
      data.thresholdValue = Number(req.body.thresholdValue);
...
export async function getDriverPackageDetail(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    res.json(await service.getDriverPackageDetail(BigInt(req.params["id"]!)));
  } catch (err) {
    next(err);
  }
}

export async function patchDriverPackageBalance(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
...
export async function acknowledgeAdminSos(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    const alert = await sosService.acknowledgeSosAlert(
      BigInt(req.params["id"]!),
      req.admin!.id,
    );
    res.json(alert);
...
export async function resolveAdminSos(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    const status =
      req.body.status === "false_alarm"
        ? ("false_alarm" as const)
        : ("resolved" as const);
    const note = req.body.note ? String(req.body.note) : undefined;
    const alert = await sosService.resolveSosAlert(
      BigInt(req.params["id"]!),
      req.admin!.id,
      status,
      note,
...
export async function getAdminDisputeTripReplay(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
...
export async function assignAdminDispute(
  ...
  try {
    res.json(
      await disputeService.assignDispute(
        BigInt(req.params["id"]!),
        req.admin!.id,
      ),
    );
  ...
  try {
    ...
    if (!body.outcome || !body.note) {
      res
        .status(400)
        .json({
          error: "outcome and note are required",
    ...
    if (body.refundAmount !== undefined)
      input.refundAmount = Number(body.refundAmount);
    res.json(
      await disputeService.resolveDispute(BigInt(req.params["id"]!), input),
    );
  } catch (err) {
    next(err);
  }
...
export async function getAdminRides(
  ...
  try {
    ...
    if (req.query["search"]) q.search = req.query["search"] as string;
    if (req.query["cashDiscrepancy"])
      q.cash_discrepancy = req.query["cashDiscrepancy"] === "true";
    if (req.query["dateFrom"]) q.date_from = req.query["dateFrom"] as string;
    if (req.query["dateTo"]) q.date_to = req.query["dateTo"] as string;
    if (req.query["cityId"])
      q.city_id = parseInt(req.query["cityId"] as string, 10);
    if (req.query["page"]) q.page = parseInt(req.query["page"] as string, 10);
    if (req.query["limit"])
      q.limit = parseInt(req.query["limit"] as string, 10);
...
export async function getUpcomingScheduledRides(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
...
export async function forceResolveAdminRide(
  ...
  try {
    ...
    const result = await service.forceResolveAdminRide(
      BigInt(req.params["id"]!),
      action,
      req.admin!.id,
      note,
...
export async function getRideAssignCandidates(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    res.json({
      candidates: await service.getRideAssignCandidates(
        BigInt(req.params["id"]!),
      ),
    });
  } catch (err) {
  ...
  try {
    ...
    if (!body.driverId || (body.mode !== "request" && body.mode !== "force")) {
      res
        .status(400)
        .json({
          error: "driverId and mode (request|force) are required",
          code: "VALIDATION_ERROR",
        });
      return;
    }
    const result = await service.assignDriverToRide(
      BigInt(req.params["id"]!),
      BigInt(body.driverId),
      body.mode,
      req.admin!.id,
      body.overrideEligibility ?? false,
    );
    res.json(result);
  } catch (err) {
    next(err);
...
export async function getAdminRentalPackages(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    ...
    if (typeof cityIdRaw === "string" && cityIdRaw !== "") {
      if (!/^\d+$/.test(cityIdRaw)) {
        res
          .status(400)
          .json({ error: "Invalid city_id", code: "VALIDATION_ERROR" });
        return;
      }
...
export async function patchAdminRentalPackage(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    ...
    if (extra_per_min !== undefined)
      fields.extra_per_min = Number(extra_per_min);
    if (is_active !== undefined) fields.is_active = Boolean(is_active);
    if (duration_minutes !== undefined)
      fields.duration_minutes = Number(duration_minutes);
    if (km_limit !== undefined) fields.km_limit = Number(km_limit);
    if (display_order !== undefined)
      fields.display_order = Number(display_order);
    if (city_id !== undefined)
      fields.city_id = city_id === null ? null : Number(city_id);
    res.json(
      await service.updateAdminRentalPackage(
        BigInt(req.params["id"]!),
        fields,
        req.admin!.id,
      ),
...
export async function deleteAdminRentalPackage(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
...
export async function postAdminRentalPackage(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
...
export async function getAdminActiveSessions(
  _req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
