...
import { fetchChatMessages, fetchRide, markChatRead, sendChatMessage, type ChatMessage } from '@/features/active-ride/api'
import { useKeyboardOffset } from '@/features/active-ride/useKeyboardOffset'
...
// silently killing every send attempt (both canned replies and the typed
// input, since both funnel through send()) with nothing visible on screen --
// no red-box in a release build, just a swallowed rejected promise.
//
...
// No join:ride here -- the active-ride screen owns that room's lifecycle
// (useActiveRide's useRoomJoin), same as web driver app's RideChat.tsx.
export default function RideChatScreen() {
  ...
  useEffect(() => {
    ...
    function onChatMessage(msg: ChatMessage) {
      ...
      upsertMessage(msg)
      if (msg.senderType === 'user') void markChatRead(rideId).catch(() => {})
    }
    ...
    function onReconnect() {
      fetchChatMessages(rideId, lastSeenIdRef.current)
        .then((caughtUp) => {
          ...
          if (caughtUp.some((m) => m.senderType === 'user')) void markChatRead(rideId).catch(() => {})
        })
        .catch(() => {})
    }
  ...
  return (
    ...
    // to compensate either. useKeyboardOffset (see its own comment) is the
    // proven-reliable fix used across this ride flow.
    <Animated.View style={[styles.container, keyboardStyle]}>
      <View style={[styles.header, { paddingTop: insets.top + spacing.sm }]}>
        ...
        {/* Generic initials avatar only -- never the rider's photo (privacy), matching web driver's RideChat.tsx. */}
        <View style={styles.headerAvatar} accessibilityElementsHidden>
          <Text style={styles.headerAvatarInitial}>{initials}</Text>
        </View>
        <View style={styles.headerInfo}>
          <Text style={styles.headerName} numberOfLines={1}>{riderName ?? 'Rider'}</Text>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  headerAvatar: { width: 36, height: 36, borderRadius: radii.full, backgroundColor: colors.primarySubtle, alignItems: 'center', justifyContent: 'center' },
  headerAvatarInitial: { ...typography.label, color: colors.primary, fontFamily: fonts.bold },
