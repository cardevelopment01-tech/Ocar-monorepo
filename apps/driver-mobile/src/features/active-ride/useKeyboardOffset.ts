...
// project -- see apps/{rider,driver}-mobile's phone.tsx HERO_HEIGHT comment,
// found the hard way in an earlier session. This uses RN core's own discrete
// Keyboard.addListener('keyboardDidShow'/'keyboardDidHide') instead, which
// isn't affected by that bug, and animates the two-state transition itself
// rather than needing a continuous native value.
//
// Needed because this app runs with edgeToEdgeEnabled (android/gradle.properties)
...
// input (an absolute-positioned sheet, or even a normal in-flow one) can end
// up sitting directly behind the keyboard with no automatic reflow at all.
