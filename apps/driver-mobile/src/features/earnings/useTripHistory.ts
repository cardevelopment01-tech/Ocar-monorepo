...
export function useTripHistory() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    setError(null)
    try {
      const [tripsRes, summaryRes] = await Promise.all([fetchTrips(1), fetchEarningsSummary()])
  ...
  return { trips, summary, loading, refreshing, loadingMore, hasMore, error, refresh, loadMore }
}
