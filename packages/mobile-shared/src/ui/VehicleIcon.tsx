 ...
 * RN port of web's VehicleIcon.tsx (apps/user/components/ui/VehicleIcon.tsx) --
 * same paths, same 80x44 viewBox, same darken-for-wheels logic. Web has 5
 * genuinely distinct silhouettes per category slug; the mobile select-ride
 * screen was showing one generic car glyph for every category before this.
 *
 * Each icon is a single monochrome silhouette using the `color` prop.
 * Wheels use a darkened shade of the same colour for contrast; windows are
 * a translucent white.
 */
...
export function VehicleIcon({ slug, size = 44, color = '#0F172A' }: VehicleIconProps) {
  const Body = BODIES[slug] ?? Sedan
  const wheel = shade(color)
  return (
    <Svg viewBox="0 0 80 44" width={size * ASPECT} height={size} fill="none">
